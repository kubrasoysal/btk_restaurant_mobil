...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _addTableDialog() async {
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(
              controller: idCtrl,
              decoration: const InputDecoration(hintText: 'Masa ID (örn: Z9)'),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: capacityCtrl,
              decoration: const InputDecoration(hintText: 'Kapasite (örn: 4)'),
    ...
    if (res != null) {
      final id = res['id']!;
      final capacity = int.tryParse(res['capacity']!) ?? 4;
      if (id.isNotEmpty) {
        AppState.addTable(activeSalon, id: id, capacity: capacity);
        setState(() {});
      }
    }
  }
